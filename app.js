...
function typeName(productId, typeId) {
  const type = productTypes(productId).find(
    item => item.id === typeId
  );

  return type?.name ||
    (typeId === "legacy" ? "قديم / غير محدد" : "غير محدد");
}
...
function productOptions(selected = "") {
  return settings.products.map(product => `
    ...
    >
      ${esc(product.name)} — ${product.boxSize} حبة
    </option>
  `).join("");
}
...
function updateFormDate() {
  ...
  if (el) {
    el.textContent = formatBusinessDate(selectedDate);
  }
}
...
function createBox(data = {}) {
  ...
  box.innerHTML = `
    <div class="mixed-box-header">
      ...
      <div class="box-header-actions">
        <button type="button" class="duplicate-box">
          نسخ
        </button>

        <button type="button" class="remove-box">
          حذف
        </button>
      </div>
    </div>
  ...
  box.querySelector(".box-product")
    .addEventListener("change", () => {
      box.dataset.product =
        box.querySelector(".box-product").value;

      renderBoxInputs(box);
      validateAll();
    });

  box.querySelector(".remove-box")
  ...
  box.querySelector(".duplicate-box")
    .addEventListener("click", () => {
...
function renderBoxInputs(box, oldItems = []) {
  ...
  const container =
    box.querySelector(".box-count-list");
...
function readBox(box) {
  ...
  const items = [
    ...box.querySelectorAll(".box-quantity")
  ]
    .map(input => ({
...
function updateBox(box) {
  ...
  box.querySelector(".box-total")
    .textContent =
    total.toLocaleString("ar-LB");

  box.querySelector(".box-max")
    .textContent =
  ...
  if (total === 0) {
    ...
    status = `صندوق جزئي · ${total} حبة`;
  }

  box.querySelector(".box-status")
    .textContent = status;
...
function getLoose() {
  return $$("#looseProducts .loose-product-row")
    .map(row => ({
      id: uid("loose"),
      productId:
        row.querySelector(".loose-product").value,
      quantity: Math.max(
        0,
        Number(
          row.querySelector(".loose-quantity").value
        ) || 0
      )
    }))
    .filter(
...
function validateAll() {
  ...
  getBoxes().forEach((box, index) => {
    ...
    if (!box.items.length) {
      valid = false;
      message =
        `أدخل محتوى الصندوق ${index + 1}.`;
    }

    if (box.total > box.boxSize) {
      valid = false;
      message =
        `الصندوق ${index + 1} يحتوي ${box.total} حبة، والحد الأقصى ${box.boxSize}.`;
    }

    const element =
      $$("#mixedBoxes .mixed-box")[index];
  ...
  if ($("#boxValidationMessage")) {
    $("#boxValidationMessage").textContent =
      message;

    $("#boxValidationMessage").hidden =
      !message;
  }
...
function resetForm() {
  ...
  $('input[name="payment"][value="cash"]').checked =
    true;
...
function filteredReport(
  ...
  return records
    .filter(record =>
      (!from ||
        record.businessDate >= from) &&
      (!to ||
        record.businessDate <= to) &&
      (type === "all" ||
        record.type === type)
    )
    .sort(
...
function renderReportPreview() {
  ...
  if (
    ...
    dateText =
      `من ${formatShortDate(from)}`;
  } else if (to) {
    dateText =
      `حتى ${formatShortDate(to)}`;
  }
  ...
  $("#reportPreview").innerHTML = `
    ...
    <div class="report-summary-grid">
      ...
      <div>
        <span>الزبائن</span>
        <strong>
          ${customerTotal.toLocaleString("ar-LB")}
        </strong>
      </div>
      ...
      <div>
        <span>الصالة</span>
        <strong>
          ${hallTotal.toLocaleString("ar-LB")}
        </strong>
      </div>
    </div>
        ...
        productTotals.length
          ? productTotals.map(item => `
              <div class="report-product-row">
                ...
                <strong>
                  ${item.quantity.toLocaleString("ar-LB")}
                  حبة
                </strong>
              </div>
            `).join("")
...
function loadSettingsForm() {
  ...
  const croissant =
    productById("croissant");

  const donut =
    productById("donut");

  $("#croissantBoxSize").value =
    croissant?.boxSize || 40;
